...
export const GROUPS: SysGroup[] = [
  ...
  { id: "agents", label: "REASONING", note: "where the outcome is decided",
    x: COL.agents - 20, y: 40, w: NW + 40, h: 470 },
  { id: "engine", label: "WORLD + LOOP", note: "facts looked up, never judged",
...
export function buildNodes(run: SimulationRun | null): SysNode[] {
  ...
  return [
    // ---------------------------------------------------------------- surface
    { id: "ui", label: "Policymaker UI", kind: "React · 9 routes", group: "surface",
      ...
      facts: [{ label: "routes", value: "9" }, { label: "transport", value: "fixture | http" }],
    ...
    { id: "voice", label: "Deliberation agent", kind: "the engine", group: "agents",
      state: "live", path: "backend/app/agents/deliberation.py",
      detail: "Residents decide for themselves what the policy does to them, in rounds, having seen what their neighbours concluded. Nothing computes the outcome first: this is the outcome. A turn citing a fact it was not given is rejected and counted.",
      facts: [{ label: "batch", value: "4 residents" }, { label: "rounds", value: "4" }],
    ...
    { id: "sim", label: "Rules engine", kind: "the fact layer", group: "engine",
      state: "live", path: "backend/app/simulation.py",
      detail: "Computes the geometry: distances, routes, journey times, and which stop a closure takes away. It no longer decides whether that constitutes harm -- the residents do, in the deliberation -- but every number on the impact and intervention screens still comes from here.",
    ...
    { id: "voices", label: "Deliberation loop", kind: "rounds · cache", group: "engine",
      state: "live", path: "backend/app/deliberate.py",
      detail: "Four rounds, bounded concurrency, cached by content hash. Each round is snapshotted before dispatch so a resident cannot hear a neighbour's same-round view. Without a model it refuses: there is no offline substitute.",
      facts: [{ label: "grounding", value: "validated per turn" }],
      x: COL.engine + NW + 36, y: 390, w: NW, h: NH },
    { id: "social", label: "Social graph", kind: "undirected", group: "engine",
      state: "live", path: "backend/app/social.py",
      detail: "Who hears whom: household, care ties, then the same road. Adapted from PropSim, plus bridges so opinion can cross the town. Without it the run is two thousand monologues rather than a deliberation.",
...
export const EDGES: SysEdge[] = [
  ...
  { from: "sim", to: "voices", kind: "data", label: "world facts" },
  { from: "pop", to: "social", kind: "data" },
  { from: "social", to: "voices", kind: "data", label: "who hears whom" },
